...
async function generateInvoicePdfAndPreview({ html, invoiceId, templateId }) {
  ...
  try {
    ...
    const launchOptions = {
      ...
      headless: true,
      // 30s limit for starting Chrome
      timeout: 30000,
      // Allow up to 3 minutes for DevTools protocol commands like Network.enable
      // (this is an upper bound; successful runs will return in a few seconds)
